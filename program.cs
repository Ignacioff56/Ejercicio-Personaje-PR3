...
Crear una clase llamada PersonajeDeMagia que tenga 2 constructores, uno que reciba los 4 atributos como parámetro y otro que reciba 3 y le ponga
el atributo magia en 60 por defecto. Al atacar debe sobreescribir el método Atacar() para que diga: “{nombre} hizo {x} de daño gracias a su magia” y 
utilizar la fórmula Fuerza+Agilidad+Magia*4 para calcular el daño.
...
PersonajeDeFuerza Guerrero = new PersonajeDeFuerza("Guillermo el guerrero", 50, 15);
PersonajeDeAgilidad Pícaro = new PersonajeDeAgilidad("Pedro el Pìcaro", 5, 80, 10);
PersonajeDeMagia Mago = new PersonajeDeMagia("Maria la Maga", 10, 30, 100);
Enemigo pEnemigo = new Enemigo(1500, 15);

IList<Personaje> personajes = new List<Personaje>();
personajes.Add(Guerrero);
personajes.Add(Pícaro);
personajes.Add(Mago);

Turno.Atacarse(personajes);

Turno.MoverFichas(Mago);
